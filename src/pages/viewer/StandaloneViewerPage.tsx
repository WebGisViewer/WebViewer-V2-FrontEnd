...
//                 try {
//                     console.log(`Loading layer: ${layerInfo.name} (ID: ${layerInfo.id})`);
//
//                     const bounds = mapRef.current!.getBounds();
//                     const zoom = mapRef.current!.getZoom();
//                     const boundsParam = `${bounds.getWest()},${bounds.getSouth()},${bounds.getEast()},${bounds.getNorth()}`;
//
//                     // Check if layer has chunked data
//                     const isChunked = layerInfo.data_source?.type === 'chunked';
//                     const chunkIds = layerInfo.data_source?.chunk_ids || [1];
//
//                     let allFeatures: any[] = [];
//
//                     if (isChunked && chunkIds.length > 1) {
//                         console.log(`Loading ${chunkIds.length} chunks for layer: ${layerInfo.name}`);
//
//                         // Load all chunks
//                         for (const chunkId of chunkIds) {
//                             try {
//                                 const chunkData = await mapService.getLayerData(layerInfo.id, {
//                                     chunk_id: chunkId,
//                                     bounds: boundsParam,
//                                     zoom: zoom
//                                 });
...
//                                     console.log(`Loaded chunk ${chunkId}: ${chunkData.features.length} features`);
//                                 }
//                             } catch (chunkError) {
//                                 console.error(`Error loading chunk ${chunkId} for layer ${layerInfo.name}:`, chunkError);
//                                 // Continue with other chunks even if one fails
//                             }
//                         }
//
//                         console.log(`Total features loaded for ${layerInfo.name}: ${allFeatures.length}`);
//                     } else {
//                         // Single chunk or non-chunked layer
...
//
//                     // Create the combined GeoJSON data object
//                     const combinedData = {
...
//
//                     console.log(`Layer ${layerInfo.name}: isTowerLayer=${isTowerLayer}, isCountyLayer=${isCountyLayer}`);
//
//                     if (isTowerLayer) {
//                         towerDataRef.current[layerInfo.id] = combinedData;
//                     }
//
...
//                         });
//
//                         L.geoJSON(combinedData, {  // Use combinedData instead of data
...
//                         mapLayer = L.geoJSON(combinedData, {  // Use combinedData instead of data
//                             style: (feature) => ({
...
//                             combinedData,  // Use combinedData instead of data
//                             layerInfo.id,
//                             layerInfo.name,
//                             companyName
//                         );
...
//                         featureCount: allFeatures.length,  // Use allFeatures.length instead of data.features.length
...
const StandaloneViewerPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const loadVisibleLayers = async () => {
            ...
            for (const layerInfo of layersToLoad) {
                try {
                    console.log(`Loading layer: ${layerInfo.name} (ID: ${layerInfo.id})`); // Debug log
                    ...
                    console.log(`Loaded ${data.features.length} features for layer: ${layerInfo.name}`);
                    ...
                    if (shouldCluster) {
                        ...
                        // Enhanced non-clustered layer processing
                                ...
                                if (feature.properties && isTowerLayer) {
                                    ...
                                    try {
                                        ...
                                        if (feature.geometry.type === 'Polygon') {
                                            ...
                                            // For multipolygon, get centroid of the largest polygon
                                        ...
                                        console.log('County centroid calculated:', { lat, lng, countyName });

                                        // Add CBRS popup to the county polygon first
                                        ...
                                        // Calculate approximate text width for better positioning
                                        const textLength = countyName.length;
                                        const approximateWidth = Math.max(textLength * 8, 80); // At least 80px wide
                                        const offsetLeft = approximateWidth / 2; // Move left by half the width
                                        const offsetUp = 10; // Move up by 10 pixels
        ...
        title="Click to view CBRS licenses for ${countyName}">${countyName}</div>`,
                                                ...
                                                iconAnchor: [offsetLeft, offsetUp], // Move left by half width, up by 10px
                                                popupAnchor: [0, 0]
                                            }),
                                            pane: 'markerPane'
                                        });
                                        ...
                                        if (mapRef.current) {
                                            countyLabel.addTo(mapRef.current);
                                            console.log('Clickable county label added to map:', countyName);
                                        }

                                        // Store reference to the label for cleanup (ONLY ONCE)
                                        const labelsKey = `${layerInfo.id}_labels`;
                                        if (!layersRef.current[labelsKey]) {
                                            layersRef.current[labelsKey] = L.layerGroup();
                                            if (mapRef.current) {
                                                (layersRef.current[labelsKey] as L.LayerGroup).addTo(mapRef.current);
                                            }
                                        }
                                        (layersRef.current[labelsKey] as L.LayerGroup).addLayer(countyLabel);

                                        // Also bind popup to the original county outline for users who click the border
                                        ...
                                        // Fallback: use feature bounds center
                                        try {
                                            const bbox = turf.bbox(feature);
                                            const centerLng = (bbox[0] + bbox[2]) / 2;
                                            const centerLat = (bbox[1] + bbox[3]) / 2;

                                            const fallbackLabel = L.marker([centerLat, centerLng], {
                                                icon: L.divIcon({
                                                    className: 'county-label-fallback',
                                                    html: `<div style="
                    ...
                    ">${countyName}</div>`,
                                            ...
                                            // Add popup to fallback label too
                                            const cbrsPopupHTML = createCBRSPopupHTML([], countyName);
                                            fallbackLabel.bindPopup(cbrsPopupHTML, {
                                                maxWidth: 650,
                                                maxHeight: 450,
                                                className: 'cbrs-popup'
                                            });
                                            ...
                                            if (mapRef.current) {
                                                fallbackLabel.addTo(mapRef.current);
                                                console.log('Fallback county label added:', countyName);
                                            }

                                        } catch (fallbackError) {
                                            console.error('Fallback label creation failed for:', countyName, fallbackError);
                                        }
                                    }
                                }
                            }
                    ...
                    console.log(`Successfully loaded ${isTowerLayer ? 'tower' : isCountyLayer ? 'county' : 'regular'} layer: ${layerInfo.name}`, {
                        isTowerLayer,
                        isCountyLayer,
                        featureCount: data.features.length,
                        cbrsLicenses: isCountyLayer ? cbrsLicenses.length : 0
                    });
            ...
            for (const loadedLayerId of Object.keys(layersRef.current).map(Number)) {
                if (!visibleLayers.has(loadedLayerId)) {
                    ...
                    // Remove county labels if they exist
                    const labelsKey = `${loadedLayerId}_labels`;
                    if (layersRef.current[labelsKey]) {
                        if (mapRef.current) {
                            mapRef.current.removeLayer(layersRef.current[labelsKey]);
                        }
                        delete layersRef.current[labelsKey];
                    }
